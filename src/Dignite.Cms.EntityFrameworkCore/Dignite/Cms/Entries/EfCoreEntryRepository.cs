...
namespace Dignite.Cms.Entries
{
    public class EfCoreEntryRepository : EfCoreRepository<ICmsDbContext, Entry,Guid>, IEntryRepository
    {
        ...
        public async Task<bool> SlugExistsAsync(Guid sectionId, string language, string slug, Guid? ignoredId = null, CancellationToken cancellationToken = default)
        {
        ...
        {
            return await (await GetDbSetAsync())
                       .AnyAsync(e => e.SectionId == sectionId && e.Language == language, GetCancellationToken(cancellationToken));
        }
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            {
                return await (await GetQueryableAsync(sectionId, language, creatorId, status, filter, start, end))
            ...
            {                
                var enumerable = (await GetQueryableAsync(sectionId, language, creatorId, status, filter, start, end))
        ...
        public async Task<int> GetMaxOrderAsync(Guid sectionId, string language, Guid? parentId, CancellationToken cancellationToken = default)
        {
            return await (await GetDbSetAsync()).Where(e => e.SectionId == sectionId && e.Language == language && e.ParentId==parentId)
